...
// monster
	var monsters = [], monsterTex, monsterMat;
	var durationP = 1470, keyframeP = 70, interpolationP = durationP / keyframeP, interpolationP2 = durationP / (keyframeP-1), lastKeyframeP = 0, currentKeyframeP = 0;
...
{	
	...
	// CIRCLE
		...
		for(var i=0; i<20; i++){
			...
			circleLight.scale.set(0.5,0.5,0.5);	//size
	...
	// MONSTER
	monsterTex = THREE.ImageUtils.loadTexture('images/monsterA.png');
	var mons = new THREE.Object3D();

		loader.load( "models/monsterA2.js", function( geometry ) {
			var monsA = new THREE.Mesh( geometry, new THREE.MeshBasicMaterial( { map: monsterTex, morphTargets: true } ) );
			monsA.position.z = -15;
			scene.add( monsA );
			monsters.push( monsA );
		} );
