...
var controls, headLight, sunLights=[];
...
// HIVE
	...
	*/
	...
	var bees = [], beeMat, pastBeeLocation = [], futureBeeLocation = [], beeToQueen = [], beeToLamp = [];
	var beeLightColors = [ 0xffbf00, 0xff6b00, 0xffc000, 0xffea00 ];
	var queen, queenMat, queenWingHigh = false;
...
{	
	...
	// web audio api
		bufferLoader = new BufferLoader(
			audioContext, [ '../audios/hive/buzzing_bee.mp3' ], 
					  finishedLoading
		);
		bufferLoader.load();
	...
	light = new THREE.DirectionalLight(0xff8000); // night: 0xff8000
	light.position.set(0, 1, 1);
	light.intensity = 0.4;	// night: 0.4
	scene.add(light);
	sunLights.push(light);

	light = new THREE.DirectionalLight(0xffeb00);// night: 0xffeb00
	light.position.set(0, 1, -1);
	light.intensity = 0.4; // night: 0.4
	scene.add(light);
	sunLights.push(light);
	...
	// beeLight
		...
		*/
		...
		for(var i=0; i<8; i++){
			...
			beeToQueen.push(false);
			beeToLamp.push(false);
		}
	...
	*/
	...
	// HIVES
		...
		// queen
			tex = THREE.ImageUtils.loadTexture('images/queen.png');
			queenMat = new THREE.MeshLambertMaterial( {map: tex, transparent: true, alphaTest: 0.5, side:THREE.DoubleSide, color: 0xffff00 } );
			loadModelQueen("models/queen.js", "models/queen_wing.js", queenMat);
	...
	lookDummy = new THREE.Mesh(geo, mat);
	lookDummy.name = 'lookDummy';
	// scene.add(lookDummy);
	...
	// LAMP
		...
		frameMat = new THREE.MeshBasicMaterial({map: woodTexture, wireframe: true, wireframeLinewidth: 2});
...
function loadModelQueen (model, modelB, meshMat) {

	queen = new THREE.Object3D();
	var loader = new THREE.JSONLoader();

	loader.load(model, function(geometry){
		var qu = new THREE.Mesh(geometry, meshMat);
		queen.add(qu);
	}, "js");

	loader.load(modelB, function(geometryB){
		var quw = new THREE.Mesh(geometryB, meshMat);
		quw.position.set(-0.2, -0.1, -0.1);
		queen.add(quw);

		queen.scale.set(3,3,3);
		queen.position.y = -30;
		// scene.add(queen);
	}, "js");
}
...
function myKeyPressed (event) {
	...
	switch ( event.keyCode ) {
		case 48: //O --> queen hand high
			queenWingHigh = !queenWingHigh;

			if(queenWingHigh) new TWEEN.Tween(queen.children[1].rotation).to({x:-30*Math.PI/180, z:-70*Math.PI/180}, 400).easing( TWEEN.Easing.Back.InOut).start();
			else new TWEEN.Tween(queen.children[1].rotation).to({x:0 , z:0}, 400).easing( TWEEN.Easing.Back.InOut).start();
		...
		case 57: //9
			...
			// console.log("ws sent sth about hive!");
			break;
		...
		case 49: //1
			...
			changeBGColor(workingMode);

			// if(workingMode){
			// 	new TWEEN.Tween(bgColor).to({r: 255, g: 222, b: 118}, 100)
			// 		.easing( TWEEN.Easing.Exponential.In).start();
			// 	renderer.setClearColor(bgColor);
			// } else {
			// 	new TWEEN.Tween(bgColor).to({r: 0, g: 0, b: 0}, 100)
			// 		.easing( TWEEN.Easing.Exponential.In).start();
			// 	renderer.setClearColor(bgColor);
			// }
			break;
...
{	
	...
	// eyeRay
		...
		eyerayCaster.set( camPos, directionCam );
		eyeIntersects = eyerayCaster.intersectObjects( scene.children, true );

		// if ( eyeIntersects.length>0 ) {
		// 	if( eyeIntersects[ 0 ].object.name == "beeMic" ) {
		// 		console.log('look at queen!');
		// 		queenWingHigh = true;
		// 	}else {
		// 		queenWingHigh = false;
		// 	}

		// 	// console.log('look at ' + eyeIntersects[ 0 ].object.name + '!');
		// 	// lookDummy.position.copy(eyeIntersects[ 0 ].object.position);
		// }

		if( eyeIntersects.length>1 ){
			// lookDummy.position.copy(eyeIntersects[ 1 ].point);

			if( eyeIntersects[ 1 ].object.name == "beeMic" ) {
				// console.log('look at queen!');
				queenWingHigh = true;
			}else {
				queenWingHigh = false;
	...
	// beeLight
		for(var i=0; i<bees.length; i++){
			...
			if(queenWingHigh && !beeToQueen[i]){

				var destinationBee = new THREE.Vector3();
				// destinationBee.setFromMatrixPosition(queen.matrixWorld);
				destinationBee.add( lamp.children[3].geometry.vertices[i] );
				destinationBee.multiplyScalar(16);
				var ddd = new THREE.Vector3();
				ddd.setFromMatrixPosition(queen.matrixWorld);
				destinationBee.add( ddd );
				// destinationBee.add(new THREE.Vector3(-6,30,-12));

				new TWEEN.Tween(bees[i].position).to({x:destinationBee.x, y:destinationBee.y, z:destinationBee.z}, 1000).easing( TWEEN.Easing.Back.InOut).start();
				beeToQueen[i] = true;

			} else if(queenWingHigh && beeToQueen[i]) {

				beeAttachToLamp(i);
				
			} else if( workingMode ) {
				bees[i].position.x = camPos.x + Math.sin( time*(4-i/2)*0.7*0.0004 ) *40;
				bees[i].position.y = camPos.y + Math.cos( time*  i  *0.5*0.0004 ) *30;
				bees[i].position.z = camPos.z + Math.cos( time*(i/2+4)*0.3*0.0004 ) *40;

				futureBeeLocation[i].x = camPos.x + Math.sin( (time+30)*(4-i/2)*0.7*0.0004 ) *40;
				futureBeeLocation[i].y = camPos.y + Math.cos( (time+30)*  i  *0.5*0.0004 ) *30;
				futureBeeLocation[i].z = camPos.z + Math.cos( (time+30)*(i/2+4)*0.3*0.0004 ) *40;

				bees[i].lookAt( futureBeeLocation[i] );

			} else if( !queenWingHigh ) {
...
function changeBGColor(status) {
	if(status){
		new TWEEN.Tween(bgColor).to({r: 0, g: 0, b: 0}, 100)
		.easing( TWEEN.Easing.Exponential.In).start();
		sunLights[0].intensity = 1;
		sunLights[1].intensity = 1;
	} else {
		new TWEEN.Tween(bgColor).to({r: 255, g: 222, b: 118}, 100)
		.easing( TWEEN.Easing.Exponential.In).start();
		sunLights[0].intensity = 0.4;
		sunLights[1].intensity = 0.4;
	}
	renderer.setClearColor(bgColor);
}

function beeAttachToLamp( beeIndex ) {

	if( !beeToLamp[beeIndex] ){
	
		setTimeout(function(){
			
			var destinationBee = new THREE.Vector3();
			destinationBee.add( lamp.children[3].geometry.vertices[beeIndex] );
			destinationBee.multiplyScalar(16);
			var ddd = new THREE.Vector3();
			ddd.setFromMatrixPosition(queen.matrixWorld);
			destinationBee.add( ddd );

			new TWEEN.Tween(bees[beeIndex].position).to({x: destinationBee.x, y: destinationBee.y, z: destinationBee.z}, 200).easing( TWEEN.Easing.Quadratic.In).start(); 
			bees[beeIndex].lookAt( destinationBee );
			beeToLamp[beeIndex] = false;

			// console.log("beeToLamp[" + beeIndex + "] = false");
		}, 200);

		beeToLamp[beeIndex] = true;
		// console.log("beeToLamp[" + beeIndex + "] = true");
	}
