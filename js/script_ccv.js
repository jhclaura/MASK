...
// WEB_CAM
	...
	var videoWidth = 240, videoHeight = 180;
	var tvsets = [];
	var tvTexture;
	var screens = [];
	var screensR = [];
...
{	
	...
	*/
	...
	// WEBCAM
		// debug view
		canvas = document.getElementById('layerOnVideo');
	...
	demo_app_BBF(videoImage.width, videoImage.height);
...
{	
	...
	*/
	...
	// ccv version
		var changed = false, scaleFactor = 1, faces;
...
// ref: https://github.com/fofr/paulrhayes.com-experiments/blob/master/webcam/js/experiment.js
function detectFaces(canvas) {
	// console.log('entered detectFaces!');
	return ccv.detect_objects({"canvas": ccv.grayscale(canvas), 
								"cascade": cascade, 
								"interval": 2,
								"min_neighbors": 1});
}

function drawMasks(context, faces) {
    if(!faces) {
        return false;
    }
    ...
    for (var i = 0; i < faces.length; i++) {
        var face = faces[i];
        context.strokeRect(face.x * 0.9, face.y * 0.9, face.width * 1.3, face.height * 1.3);
    }
}
