	...
	var onKeyDown = function ( event ) {
		...
		switch ( event.keyCode ) {
			...
			case 49: //1
				...
				if(workingMode){
					new TWEEN.Tween(yawObject.position).to({x: 34, y: 1, z:375}, 1000).easing( TWEEN.Easing.Back.InOut).start();
				} else {
					new TWEEN.Tween(yawObject.position).to({x:myStartX, y:myStartY, z:myStartZ}, 1000).easing( TWEEN.Easing.Back.InOut).start();
				}
				break;
