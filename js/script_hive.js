...
{	
	...
	// Cam Material
		for (var i=0; i<bigCellFiles.length; i++){
			...
			if ( i==myIndex ) {
				...
				cellMats.push(rMat);
				// console.log(rMat.map);

			// if it's remote --> remoteCam
			// if whoIamInHive==, i here will be 0~3, can safely use remoteTextures
			} else if ( i<myIndex ) {
				// console.log("i: " + i);
	...
	// HIVES
		...
		loader.load( bigCellFiles[0], function( geometry ) {
			var c = new THREE.Mesh( geometry.clone(), cellMats[0] );
			cells.push(c);
			scene.add(c);
		} );
		loader.load( bigCellFiles[1], function( geometry ) {
			var c = new THREE.Mesh( geometry.clone(), cellMats[1] );
			cells.push(c);
			scene.add(c);
		} );
		loader.load( bigCellFiles[2], function( geometry ) {
			var c = new THREE.Mesh( geometry.clone(), cellMats[2] );
			cells.push(c);
			scene.add(c);
		} );
		loader.load( bigCellFiles[3], function( geometry ) {
			var c = new THREE.Mesh( geometry.clone(), cellMats[3] );
			cells.push(c);
			scene.add(c);
		} );
		loader.load( bigCellFiles[4], function( geometry ) {
			var c = new THREE.Mesh( geometry.clone(), cellMats[4] );
			cells.push(c);
			scene.add(c);
		} );
