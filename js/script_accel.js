...
// Gyro
	var arm, accelReading = {}, accelToRotation = {}, magReading = {}, magToRotation = {};
	var armPitch, armYaw, armRoll;
	var armAlignQuaternion, armEuler, armQuaternion, armQuaternionLerp, armFinalQ;
...
{	
	...
	*/
	...
	// ARM
		armPitch = new THREE.Object3D();
		armYaw = new THREE.Object3D();
		armRoll = new THREE.Object3D();
		armEuler = new THREE.Euler();
		armAlignQuaternion = new THREE.Quaternion();
		armQuaternion = new THREE.Quaternion();
		armQuaternionLerp = new THREE.Quaternion();
		armFinalQ = new THREE.Quaternion();

		g_mat = new THREE.MeshLambertMaterial( {map: g_tex} );
		loadModelArm("models/arrow.js", g_mat);
...
function loadModelArm (model, meshMat) {

	var loader = new THREE.JSONLoader();
	loader.load(model, function(geometry){
		arm = new THREE.Mesh(geometry, meshMat);
		// arm.scale.set(1,1,5);

		armPitch.add(arm);
		armRoll.add(armPitch);
		armYaw.add(armRoll);
		scene.add(armYaw);
	...
	*/
	...
	// move ARM
		if(arm){
			...
			// var armRotX = Math.atan2(accelReading.y, accelReading.z);
			// var armRotY = Math.atan2(-accelReading.x, Math.sqrt( Math.pow(accelReading.y, 2) + Math.pow(accelReading.z, 2) ));

			//set #-1
			// var Roll   = Math.atan2(  accelReading.y ,  Math.sqrt( Math.pow(accelReading.x, 2) + Math.pow(accelReading.z, 2) ));  
 		// 	// var Pitch  = Math.atan2(  accelReading.x ,  Math.sqrt( Math.pow(accelReading.y, 2) + Math.pow(accelReading.z, 2) ));
 		// 	var Pitch = Math.atan2( -accelReading.x, accelReading.z );
			...
			// set #1 - Rxyz
			// var Roll = Math.atan2( accelReading.y, accelReading.z );
			// var Pitch = Math.atan2(  -accelReading.x ,  Math.sqrt( Math.pow(accelReading.y, 2) + Math.pow(accelReading.z, 2) ));

			// set #1 -  Ryxz, from an arbitrary accelerometer reading
			// var Roll = Math.atan2(  accelReading.y ,  Math.sqrt( Math.pow(accelReading.x, 2) + Math.pow(accelReading.z, 2) ));
			// var Pitch = Math.atan2( -accelReading.x, accelReading.z );

			var Yaw    = magReading.h + (30*Math.PI/180);

			// low pass filter!
				// Roll = data.x * filterAlpha + (accelReading.x * (1.0 - filterAlpha));
				// Pitch = data.y * filterAlpha + (accelReading.y * (1.0 - filterAlpha));
				// Yaw = data.z * filterAlpha + (accelReading.z * (1.0 - filterAlpha));

			armEuler.set(-Pitch, -Yaw, Roll, 'YXZ');
			armQuaternion.setFromEuler(armEuler);
			armQuaternionLerp.slerp(armQuaternion, 0.8);

			armFinalQ.copy(armAlignQuaternion);
			armFinalQ.multiply(armQuaternionLerp);

			armYaw.rotation.setFromQuaternion( armFinalQ );



			// console.log("Roll: " + Roll + ", Pitch: " + Pitch);
			// console.log("magReading.x: " + magReading.x + "magReading.y: " + magReading.y + "magReading.z: " + magReading.z);

			// // armYaw.rotation.y = Yaw;
			// armPitch.rotation.x = -Roll;
			// // armRoll.rotation.z = Pitch;
			// armYaw.rotation.y = -Yaw;
