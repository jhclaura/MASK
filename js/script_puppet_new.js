...
{	
	...
	*/
	...
	g_tex.minFilter = THREE.LinearFilter;
	// PUPPET & YOU
		boyOnPuppetTex = THREE.ImageUtils.loadTexture('images/boyBack.png');
		boyOnPuppetTex.minFilter = THREE.LinearFilter;	
	...
	// mushroom!
		...
		// mushroomTex.minFilter = THREE.LinearFilter;
		for(var i=0; i<15; i++){
	...
	//RAIN
		...
		rain.minFilter = THREE.LinearFilter;
		...
		rainMat = new THREE.PointsMaterial({ size: 1.5, map: rain, blending: THREE.AdditiveBlending, depthTest: false, transparent: true});
		rainParticle = new THREE.Points(rainGeo, rainMat);
	...
	loader.load(model, function(geometry){
		...
		groundLowerGeo = new THREE.BufferGeometry().fromGeometry( groundLowerGeo );
			...
			for ( var f = 0; f < numFaces; f ++ ) {
				...
				var h = 0.45 * Math.random();
				var s = 0.5 + 0.5 * Math.random();
				var l = 0 + 0.5 * Math.random();

				color.setHSL( h, s, l );

				var d = 20 * ( 0.5 - Math.random() );
...
function loadModelMushroom (model, index) {
	...
	loader.load(model, function(geometry, material){
		...
		for(var i=0; i<material.length; i++){
			...
			// m.color.setHSL( 0.1*index, 0.7, 0.3 );
	...
	switch ( event.keyCode ) {
		...
		case 70: //F
			if(detectFaceMode){
				...
				if(reallySeeFace) sample.trigger(15, 2);
			...
			if(detectFaceMode) renderer.setClearColor( 0xf8a3c4, 1);
...
function update() {	
	...
	*/
	...
	//RAIN
		...
		rainParticle.geometry.dynamic = true;
		rainParticle.geometry.verticesNeedUpdate = true;
	...
	// ground stuff
	if(uniforms)
		uniforms.amplitude.value = 1.0 + Math.sin( time * 0.0005 );
...
function puppetMode(type){
	if(type==1){
		...
		if(detectFaceMode){
			...
			renderer.setClearColor( 0xf8a3c4, 1);
		}
