...
// computer vision
	...
	var seeFaceTime, noFaceTime, enterConversationTime, faceDuration = 3000;
	var conIndex = 0;
...
var rain, rainGeo, rainMat, rainParticle, rainVelocity=[];
...
{	
	...
	// GROUND
		...
		loadModelGround( 'models/terrain2.js', new THREE.MeshLambertMaterial( {color: 0xcccccc, shading: THREE.FlatShading, side: THREE.DoubleSide} ) );
	...
	// STAR
		// star = new THREE.PointLight(0xffff00, 1, 30);
			...
			for(var i=0; i<100; i++){
				...
				st.position.set( Math.random()*600-300, Math.random()*-100+400, Math.random()*600-300);
	...
	*/
	...
	// WEBCAM
		...
		// puppet version eye screen
			setTimeout(function(){
				eyeGeo = new THREE.PlaneGeometry(0.3, 0.2, 1, 1);
				puppetEye = new THREE.Mesh(eyeGeo, tvMat);
				puppetEye.position.set(0,3,0.2);
				puppetEye.rotation.x = 15*Math.PI/180;
				pupp.add(puppetEye);
			}, 3000);
			// eyeGeo = new THREE.PlaneGeometry(6, 4, 1, 1);
			// puppetEye = new THREE.Mesh(eyeGeo, tvMat);
			// puppetEye.position.set(0,0,-3);
			// pupp.add(puppetEye);
	...
	//RAIN
		...
		rainMat = new THREE.PointCloudMaterial({ size: 1, map: rain, blending: THREE.AdditiveBlending, depthTest: false, transparent: true});
...
function draw_faces(ctx, rects, sc, max) {
    ...
    n = Math.min(n, on);
    	...
    	// AUDIO!!
	    	...
	    	if( (enterConversationTime+faceDuration < Date.now()) && reallySeeFace && !enterConversation){
	    	...
	    	if( (noFaceTime+faceDuration < Date.now()) && !seeFace && !reallyNoFace){
	    		reallyNoFace = true;
	    		reallySeeFace = false;
	    		enterConversation = false;
	    		conIndex = 0;
